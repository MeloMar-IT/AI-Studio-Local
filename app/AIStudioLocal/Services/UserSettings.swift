...
public final class UserSettings: ObservableObject {
    ...
    private func setupDefaultDirectories() {
        ...
        // If the worker script path is empty OR doesn't exist, try to find it
        if workerScriptPath.isEmpty || !FileManager.default.fileExists(atPath: workerScriptPath) {
            let foundPath = fileSystem.getDefaultWorkerScriptPath()
            // Only update if we found something that isn't the fallback OR if current is empty
            if !foundPath.hasPrefix("/usr/local/bin") || workerScriptPath.isEmpty {
                workerScriptPath = foundPath
            }
        }
    }
