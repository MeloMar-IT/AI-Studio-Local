...
@pytest.mark.asyncio
async def test_dog_generation_real():
    """
    End-to-end test producing a 2-second video of a running dog.
    ...
    request = GenerationRequest(
        prompt="A energetic golden retriever running joyfully across a sunny park, realistic, high quality, cinematic, 2 seconds",
    ...
    print(f"\nStarting dog generation job: {job_id}")
    ...
    # 5. Verifications
        ...
        print(f"\nSUCCESS: Dog video generated at {result_path}")

    except Exception as e:
        print(f"\nFAILURE: Dog generation failed: {e}")
        raise e

if __name__ == "__main__":
    asyncio.run(test_dog_generation_real())
